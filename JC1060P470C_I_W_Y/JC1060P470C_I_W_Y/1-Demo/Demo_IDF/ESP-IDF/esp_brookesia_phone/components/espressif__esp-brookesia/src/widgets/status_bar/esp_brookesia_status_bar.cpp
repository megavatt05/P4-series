...
ESP_Brookesia_StatusBar::ESP_Brookesia_StatusBar(const ESP_Brookesia_Core &core, const ESP_Brookesia_StatusBarData_t &data, int battery_id,
    ...
    _wifi_id(wifi_id),
    _ethernet_id(ethernet_id),
    _is_ethernet_initialed(false),
    _clock_hour(-1),
    _clock_min(-1),
...
bool ESP_Brookesia_StatusBar::begin(lv_obj_t *parent)
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_GOTO(beginEthernet(), err, "Begin battery failed");
        ...
        ESP_Brookesia_StatusBarData_t &data)
{
    ...
    //ethernet
    if(data.flags.enbale_ethernet_icon) {
        ESP_BROOKESIA_LOGD("Calibrate Ethernet icon data");
        if (data.flags.enable_ethernet_icon_common_size) {
            data.ethernet.icon_data.size = data.icon_common_size;
        }
        ESP_BROOKESIA_CHECK_FALSE_RETURN(calibrateIconData(data, home, data.ethernet.icon_data), false,
                                         "Calibrate Ethernet icon data failed");
    }
...
bool ESP_Brookesia_StatusBar::beginEthernet(void)
{
    ESP_Brookesia_LvObj_t ethernet_icon = nullptr;
    ESP_BROOKESIA_LOGD("Begin battery(0x%p)", this);
    ESP_BROOKESIA_CHECK_FALSE_RETURN(!checkEthernetInitialized(), false, "Already initialized");

    ESP_BROOKESIA_CHECK_FALSE_RETURN(addIcon(_data.ethernet.icon_data, _data.ethernet.area_index, _ethernet_id), false,
                                     "Add ethernet icon failed");
    ESP_BROOKESIA_CHECK_FALSE_GOTO(setEthernetIconState(0), err, "Set wifi state failed");

    _is_ethernet_initialed = true;
    return true;
err:
    ESP_BROOKESIA_CHECK_FALSE_RETURN(removeIcon(_ethernet_id), false, "Delete wifi failed");
    return false;
}

bool ESP_Brookesia_StatusBar::setEthernetIconState(int state) const
{
    ESP_BROOKESIA_LOGD("Set ethernet icon state(0x%p: %d)", this, state);
    ESP_BROOKESIA_CHECK_FALSE_RETURN(setIconState(_ethernet_id, state), false, "Set wifi icon state failed");
...
{
    ESP_BROOKESIA_LOGD("Set clock(%02x:%02x %s)", hour, minute, is_pm ? "PM" : "AM");
    ESP_BROOKESIA_CHECK_NULL_RETURN(_clock_obj, false, "Invalid clock");

    hour = max(min(hour, 0x23), 0);
    minute = max(min(minute, 0x59), 0);

    if (_clock_hour != hour) {
        _clock_hour = hour;
        lv_label_set_text_fmt(_clock_hour_label.get(), "%02x", hour);
    }
    if (_clock_min != minute) {
        _clock_min = minute;
        lv_label_set_text_fmt(_clock_min_label.get(), "%02x", minute);
    }
