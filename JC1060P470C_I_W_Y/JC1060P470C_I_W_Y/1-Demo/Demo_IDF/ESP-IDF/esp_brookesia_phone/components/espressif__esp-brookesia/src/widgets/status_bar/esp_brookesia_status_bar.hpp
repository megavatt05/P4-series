...
public:
    ...
    ESP_Brookesia_StatusBar(const ESP_Brookesia_Core &core, const ESP_Brookesia_StatusBarData_t &data, int battery_id, int wifi_id,int ethernet_id);
    ...
    //ethernet
    bool setEthernetIconState(int state) const;
...
private:
    ...
    bool beginEthernet(void);
    bool checkEthernetInitialized(void) const { return _is_ethernet_initialed; }
    ...
    ESP_Brookesia_LvObj_t _ethernet_icon;
    // Wifi
    int _wifi_id;
    //ethernet
    int _ethernet_id;
    bool _is_ethernet_initialed;
    // Clock
